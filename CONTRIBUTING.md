...
Releases are automated with [semantic-release](https://github.com/semantic-release/semantic-release). Every push to `main` is analyzed for Conventional Commits since the last release: a `fix:` cuts a patch, a `feat:` cuts a minor, and a `BREAKING CHANGE` footer (or `!`) cuts a major. If nothing warrants a release, nothing happens. If something does, it bumps `package.json` and `CHANGELOG.md`, builds `dist/` and commits it (see below), commits that to `main`, tags the release (e.g. `v8.1.0`), publishes the GitHub Release, and moves the floating major tag (e.g. `v8`) to point at it. No manual step, no per-PR file to write. The release configuration and workflow are managed centrally in [igorjs-iac/repo-config](https://github.com/igorjs-iac/repo-config) rather than committed to this repository, following the same pattern as `.github/CODEOWNERS` and the other files carrying a "Managed by" header; don't look for `.releaserc.json` here, and don't try to hand-roll one.

### `dist/` is release-only

`dist/index.js`, the bundle GitHub's `node24` runtime actually executes (`action.yml`'s `runs.main`), is gitignored everywhere except on tagged release commits. It is **not** committed on regular pushes to `main`, and PRs should never touch it or need to resolve conflicts on it: `pnpm run build` produces it locally for testing (`test:e2e` depends on a fresh local build), but it isn't tracked until the release pipeline builds and commits it as part of cutting a release. [`check-dist.yml`](.github/workflows/check-dist.yml) is the supply-chain check for that step: on every pushed release tag, it rebuilds from that tag's own source and fails if the committed `dist/` doesn't match. One consequence: `@main` and arbitrary non-release commit SHAs aren't valid ways to pin this action, since only release tags carry a built `dist/`; see [Versioning](README.md#versioning) in the README.
